/* Restore/keep "The Bravery" as in WotLK (not Vanilla) to avoid client modifications */
...
/* Removes Zep between Orgrimmar and Thunder Bluff added in late WotLK (3.2.0) */
DELETE FROM `transports` WHERE `guid`= 20;
DELETE FROM `creature` WHERE `guid` IN (203461, 203490);
